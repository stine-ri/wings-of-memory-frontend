...
import { Search, Filter, Eye, ChevronDown, X, Loader2, Facebook, Twitter, Instagram, Mail, Menu } from 'lucide-react';
...
export default function SearchNavbar({ 
  ...
  useEffect(() => {
    const checkMobile = () => {
      ...
      if (mobile && document.activeElement === inputRef.current) {
        setIsExpanded(true);
      }
    };
    ...
    const handleFocus = () => {
      if (isMobile && inputRef.current === document.activeElement) {
  ...
  // Scroll to keep input visible on mobile when typing
  useEffect(() => {
    if (isMobile && isInputFocused && inputRef.current) {
      const scrollToInput = () => {
  ...
  }, [isMobile, isInputFocused, searchQuery]);
  ...
  const handleSearch = () => {
    ...
    // On mobile, keep expanded view after search so user can see results
    if (!isMobile) {
      if (inputRef.current) {
  ...
  const handleKeyPress = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      handleSearch();
      // On mobile, keep focus after Enter to see what was typed
  ...
  const handleInputBlur = () => {
    setIsInputFocused(false);
    // Don't collapse on mobile if there's search text or results are showing
    if (isMobile && (searchQuery || showResultsDropdown)) {
      // Keep expanded on mobile when user is interacting with search
    }
  };
  ...
  // Handle mobile back button/gesture to collapse
  useEffect(() => {
  ...
  }, [showResultsDropdown, isExpanded, isMobile]);

  return (
    <div 
      ref={containerRef}
      className={`bg-gradient-to-r from-amber-50 to-orange-50 border-b border-amber-200 shadow-sm sticky top-0 z-50 ${
        isMobile && isExpanded ? 'min-h-screen' : ''
      }`}
    >
      <div className={`max-w-7xl mx-auto px-3 sm:px-4 transition-all duration-300 ${
        isExpanded ? 'py-3' : 'py-3 sm:py-4'
      }`}>
        ...
        <div className="flex items-center justify-between gap-3 sm:gap-4">
          ...
          <div className={`shrink-0 ${isMobile && isExpanded ? 'hidden' : ''}`}>
            ...
            <div className="flex items-center justify-between w-full mb-4">
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="p-2 text-gray-700 hover:text-gray-900 hover:bg-amber-100 rounded-lg transition-colors"
                  aria-label="Back"
                >
                  <ChevronDown className="w-5 h-5 rotate-90" />
                </button>
                <div>
                  <h1 className="text-lg font-bold text-gray-900">Search Memorials</h1>
                  <p className="text-sm text-gray-600">Find and honor loved ones</p>
                </div>
              </div>
              {searchQuery && (
                <button
          ...
          <div className={`relative ${isMobile && !isExpanded ? 'flex-1 max-w-none' : 'flex-1 max-w-2xl'}`}>
            {isMobile && !isExpanded ? (
              ...
              <button
                ...
                className="w-full flex items-center justify-between px-4 py-2.5 bg-white border-2 border-amber-300 rounded-xl shadow-sm"
              >
                <div className="flex items-center gap-2 text-gray-500">
                  <Search className="h-4 w-4" />
                  <span className="text-sm truncate">Search memorials...</span>
                </div>
                <ChevronDown className="h-4 w-4 text-gray-400" />
              </button>
            ) : (
              // Full search bar (expanded on mobile, always on desktop)
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={isMobile ? "Search by name, location, or story..." : "Search memorials by name, location, or story..."}
                  className="block w-full pl-10 sm:pl-12 pr-20 sm:pr-28 py-3 sm:py-3.5 border-2 border-amber-300 
                ...
                <div className="absolute inset-y-0 right-0 flex items-center pr-1 sm:pr-2">
                  {searchQuery && (
                    <button
                      onClick={clearSearch}
                      className="p-2 sm:p-2.5 mr-1 sm:mr-2 text-gray-500 hover:text-gray-700 transition-colors"
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 sm:gap-2 px-4 sm:px-5 py-2.5 sm:py-3 bg-amber-500 hover:bg-amber-600 
            ...
            {showResults && showResultsDropdown && (
              <div 
                ...
                className={`absolute top-full left-0 right-0 mt-2 bg-white border-2 border-amber-300 
                         ...
                         } animate-fade-in mobile-results-dropdown`}
                style={{
          ...
          {/* Filter & Menu Buttons - Mobile Optimized */}
          {!isExpanded && (
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              {isMobile && (
                <button
                  ...
                  className="p-2.5 text-gray-700 hover:text-gray-900 hover:bg-amber-100 rounded-lg transition-all border-2 border-amber-300 bg-white"
                  aria-label="Menu"
                >
              ...
              {/* Filter Dropdown - Hidden on mobile when menu is active */}
              {(!isMobile || !showMobileMenu) && (
                <div className="relative">
                  <button
                    onClick={() => setShowFilters(!showFilters)}
                    className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2.5 text-sm sm:text-base text-gray-700 hover:text-gray-900 
              ...
              {/* Expand/Collapse Button - Hidden on mobile in favor of auto-expand */}
              {!isMobile && (
                <button
        ...
        {isMobile && showMobileMenu && (
          <div className="mobile-menu-container absolute top-full left-0 right-0 mt-2 bg-white border-2 border-amber-300 rounded-xl shadow-2xl z-50 p-4 animate-fade-in">
            <div className="space-y-2">
              ...
              <div className="pt-2 border-t border-gray-200">
                <button
                  ...
                  className="w-full text-left px-4 py-3 rounded-lg transition-colors text-base hover:bg-amber-50 text-gray-800"
                >
                  Advanced Search
                </button>
              </div>
            ...
            <div className="mb-4">
              <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-sm sm:text-base text-gray-700">
                ...
                <button
                  ...
                  className="px-3 sm:px-4 py-2 sm:py-2 bg-amber-100 hover:bg-amber-200 rounded-lg text-gray-800 font-medium transition-colors text-sm sm:text-base"
                >
                  Name
                </button>
                <button
                ...
                {isMobile && (
                  <button
                    ...
                    className="ml-auto text-sm text-amber-700 hover:text-amber-800 font-bold px-3 py-2 bg-amber-100 rounded-lg"
                  >
                    Close
                  </button>
                )}
                <button
                  onClick={clearSearch}
                  className="text-sm sm:text-base text-amber-700 hover:text-amber-800 font-bold"
                >
                  Clear all
                </button>
              </div>
      ...
      <style >{`
        @media (max-width: 640px) {
          .mobile-search-input {
            ...
            height: 52px; /* Taller for better touch */
            padding-left: 44px !important; /* More space for icon */
            padding-right: 76px !important; /* More space for buttons */
          }
          ...
          .mobile-results-dropdown {
            ...
            max-height: calc(100vh - 160px) !important; /* Account for keyboard */
          ...
          @media (max-height: 600px) {
            .mobile-search-input {
              font-size: 18px !important; /* Larger text on small screens */
            }
